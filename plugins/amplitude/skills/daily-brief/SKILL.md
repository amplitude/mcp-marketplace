...
4. **Experiment check (1-2 calls).** Call `use_amp_experiments` once. Only call `use_amp_experiments` for experiments that appear to have changed status recently or that the user owns. Skip querying experiments that are clearly irrelevant.
5. **Feedback (2 calls).** Call `use_amplitude_ai_feedback` once to get sourceIds, then call `use_amplitude_ai_feedback` once with the most relevant sourceId. Focus on feedback from the last 1-2 days. Surface new or spiking themes, especially anything that appeared for the first time yesterday or today.
6. **Deployment context.** If the user supplies recent release information, use it as a hypothesis for metric movements — never as proof of causation.

...
1. Call `Amplitude:use_amp_experiments` with `action: "get"` in batches of 3-5 IDs (only the filtered set from Step 1).
...
- **Query error from `use_amp_experiments`:** Report the error clearly with the experiment owner's name. Flag as "Fix config" in the summary. Common causes: broken exposure event property, misconfigured experiment key.

...
- Search for relevant events: `Amplitude:search_amp_entities` with entityTypes: ["EVENT", "CUSTOM_EVENT"]
...
- Use `Amplitude:get_amplitude_charts` to retrieve their definitions
...
- Save all charts with `Amplitude:render_amplitude_chart` before building dashboard
...
- Save in batches using `render_amplitude_chart` (more efficient)
...
Use `Amplitude:use_amp_dashboards` with:

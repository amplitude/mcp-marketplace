...
Use `Amplitude:use_amplitude_ai_feedback` to see which feedback channels are connected (surveys, support, app reviews, etc.).

### Step 2: Get Themed Insights

Use `Amplitude:use_amplitude_ai_feedback` with appropriate filters:
...
For top insights, use `Amplitude:use_amplitude_ai_feedback` to see the actual user feedback driving each theme.

### Step 4: Connect to User Segments

Use `Amplitude:use_amplitude_cohorts` to understand if feedback themes correlate with:

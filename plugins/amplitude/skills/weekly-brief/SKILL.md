...
4. **Experiment check (1-2 calls).** Call `use_amp_experiments` once. Focus on experiments that concluded this week, hit significance this week, or have been running >14 days without a call. Only call `use_amp_experiments` for the most relevant ones.
5. **Feedback (2 calls).** Call `use_amplitude_ai_feedback` once to get sourceIds, then call `use_amplitude_ai_feedback` once with the most relevant sourceId. Focus on themes that emerged or intensified over the past week — not individual data points.
6. **Deployment context.** If the user supplies release information, use it to contextualize metric movements; never infer causation from timing alone.
...
1. **Check for false positives:**
   ...
   - **Launch phase growth**: Consider user-supplied release or flag-ramp context as a possible explanation for expected growth.
...
1. **Fact-check**: Review the data already fetched for the 2-3 most consequential claims. Only re-query with `query_amplitude_data` or `query_amplitude_data` if you're genuinely uncertain about a number — not as a routine step. If the data came from a `query_amplitude_data` result, trust it.
...
Cause: Called `use_amplitude_ai_feedback` without first calling `use_amplitude_ai_feedback`, or passed multiple values in the `types` array.
Solution: ALWAYS call `use_amplitude_ai_feedback` first. Only pass a single type value per call, or omit the types parameter entirely.

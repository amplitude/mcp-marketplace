...
2. **Get AI schema.** Call `Amplitude:get_amplitude_agent_analytics_info` with `include: ["filter_options"]` to discover available agent names, tool names, topic models, and rubric definitions. This tells you what's in the data before you query it.
...
1. **Quality + cost + performance overview.** Call `Amplitude:get_amplitude_agent_analytics_info` with `metrics: ["quality", "cost", "performance", "agent_stats", "error_categories", "rubric_scores"]`. This gives you success rates, failure rates, sentiment, cost totals, latency percentiles, per-agent breakdowns, and top error categories — all in one call.

2. **Time series trends.** Call `Amplitude:get_amplitude_agent_analytics_info` with `metrics: ["quality_timeseries", "volume_timeseries", "cost_timeseries", "success_rate_timeseries", "sentiment_timeseries", "latency_timeseries"]` and `interval: "DAY"`. This gives you the trend lines to spot regressions and spikes.

3. **Recent failures.** Call `Amplitude:get_amplitude_agent_analytics_info` with `hasTaskFailure: true`, `limit: 10`, `orderBy: "-session_start"`, `responseFormat: "concise"`. This gives you the most recent failed sessions for drill-down examples.

4. **Frustrated users.** Call `Amplitude:get_amplitude_agent_analytics_info` with `maxSentimentScore: 0.4`, `limit: 10`, `orderBy: "-session_start"`, `responseFormat: "concise"`. This surfaces sessions where users were unhappy.
...
1. **For error spikes:** Call `Amplitude:get_amplitude_agent_analytics_info` filtered to the relevant agent or error pattern with `responseFormat: "detailed"`, `limit: 5` to get full enrichment data including failure reasons and rubric scores.

2. **For quality regressions:** Call `Amplitude:get_amplitude_agent_analytics_info` with `maxQualityScore: 0.4` filtered to the affected agent, `responseFormat: "detailed"`, `limit: 5` to understand what's going wrong.

3. **For cost anomalies:** Call `Amplitude:get_amplitude_agent_analytics_info` with `groupBy: ["model_name"]` to see cost breakdown by model, or filter to the expensive agent to see which tools/models drive cost.

...
1. **Deployment correlation.** If the user supplies release context, check whether error spikes align with recent deploys. A release within 24 hours is a leading hypothesis, not proof.
2. **Experiment correlation.** If the user mentions an experiment or if errors concentrate in a segment that maps to an experiment variant, call `Amplitude:use_amp_experiments` and `Amplitude:use_amp_experiments` to check.
...
4. **Feedback correlation.** Call `Amplitude:use_amplitude_ai_feedback` then `Amplitude:use_amplitude_ai_feedback` with keywords from the top error messages. If users are reporting the same issue, it validates the impact and may provide additional context the data can't.
...
Actions:
1. Get context and use any user-supplied release information

...
Use `Amplitude:query_amplitude_data` to fetch data for up to 3 charts at a time. Prioritize:
...
1. Use `Amplitude:use_amplitude_ai_feedback` with:
...
3. If a relevant insight is found, use `Amplitude:use_amplitude_ai_feedback` with the `insightId` to pull specific user quotes that illustrate the pattern.
